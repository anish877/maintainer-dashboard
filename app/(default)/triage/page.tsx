...
export default function TriagePage() {
  ...
  const handleRepoChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedRepo = repos.find(repo => repo.fullName === e.target.value)
    if (selectedRepo) {
      setFormData(prev => ({
        ...prev,
          ...
          <div className="flex items-center">
            <div className="relative">
              <img
                ...
                src={session.user.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(session.user.name || 'User')}&background=8b5cf6&color=fff`}
              ...
              {reposLoading ? (
                ...
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
                  <p className="text-sm text-red-700 dark:text-red-300">
                    Failed to load repositories. Please enter manually below.
                  </p>
                </div>
              ) : (
                <select
                ...
                >
                  ...
                  {repos.map((repo) => (
                    <option key={repo.id} value={repo.fullName}>
                      {repo.fullName} {repo.visibility === 'Private' ? '🔒' : '🌐'} ({repo.openIssues} issues)
                    </option>
                  ))}
                </select>
              )}
